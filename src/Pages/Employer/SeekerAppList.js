...
const SeekerAppList = ({ p, index, setOpenModal }) => {
    ...
    return <><tbody key={_id} className='border-b'>
        <tr>
            ...
            <td className='py-0'>
                <button onClick={() => handleCLetter(coverLetter, subject)} className='btn btn-link pl-0 pr-2 normal-case text-sm'>Cover-letter</button>|
                <button onClick={() => navigate('/dashboard/seeker-resume', {
                    state:
                        { resume: resume, seekerName: seekerName }
                })} className='btn btn-link pl-2 pr-0 normal-case text-sm'>Resume</button>
            </td>
        </tr>
    </tbody>
    </>;
